...
class MainWidget(Widget):
    ...
    from user_actions import on_touch_up, on_touch_down, on_keyboard_up, on_keyboard_down, _keyboard_closed

    perspective_point_x = NumericProperty(0)
    perspective_point_y = NumericProperty(0)

    V_NB_LINES = 8
    V_LINES_SPACING = .4  # percent in screen width
    vertical_lines = []

    H_NB_LINES = 15
    H_LINES_SPACING = .1  # percent in screen height
    horizontal_lines = []
    current_offset_y = 0
    current_offset_x = 0
    current_speed_x = 0
    ...
    def __init__(self, **kwargs):
        ...
        # print(" SIZE ON PIXEL :[ " + str(self.width) + ", " + str(self.height) + " ]")
        if self.is_desktop():
